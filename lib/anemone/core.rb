...
module Anemone
  ...
  class Core
    ...
    def initialize(urls, opts = {})
      ...
      @valid_domains = @urls.map{|u| [u.host, URI.parse(URI.encode(u.to_s)).host.gsub(/^www\./,'')]}.flatten.compact.uniq
    ...
    def in_allowed_subdomain?(link)
      opts[:crawl_subdomains] and @valid_domains.find{|domain| URI.parse(URI.encode(link.to_s)).host.end_with?(domain)}
    end
